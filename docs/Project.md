...
- Hardening-этап должен поддерживать управляемое отключение через параметр `security_hardening_enabled` для диагностических контуров.
...
- security_hardening: `Disabled` в текущем диагностическом контуре (`security_hardening_enabled: false`)
