...
## Дата: 2026-03-02 (сессия 28)
### Наблюдения
- Обнаружен дрейф между документацией и фактическим состоянием репозитория по ingress: в ролях и inventory используется `node_ip` режим, а в части документов оставалось описание основного `LoadBalancer`-сценария.
- Подтверждено, что proxy-конфигурация для `containerd` реализована через systemd drop-in в роли `proxy`.
...
### Проблемы
- Требуется стендовая проверка доступности ingress по `node_ip` на `80/443` из клиентской сети после следующих запусков `bootstrap/validate`.

## Дата: 2026-03-03 (сессия 29)
### Наблюдения
- Получено дополнительное эксплуатационное требование явно контролировать заполнение `hosts` на узлах.
- В коде это уже реализовано в `base_os` через управление `/etc/hosts` на основе inventory.
...
## Дата: 2026-03-03 (сессия 31)
### Наблюдения
- После разрешения merge-конфликта в документации оставался риск повторяющихся шагов в `Tasktracker/changelog`.

### Решения
- Проведена ревизия документации на повторы шагов и conflict-маркеры.
- Удалены повторяющиеся конфликтные блоки в `Tasktracker/changelog`, сохранены только уникальные шаги.
- Результат зафиксирован в трекере как задача `T-074`.

### Проблемы
- Критичных проблем не выявлено.

## Дата: 2026-03-03 (сессия 32)
### Наблюдения
- Получен запрос на отключение firewall и исключение лишних firewall-шагов из выполнения ролей в текущем troubleshooting-контуре.
- В базовой роли `base_os` пакет `firewalld` устанавливался всегда, даже при отключенном firewall-флаге.

### Решения
- Добавлена и закрыта задача `T-075`: `firewalld_enabled` зафиксирован в `false` на уровне inventory.
- В `security_hardening` добавлено явное `stopped/disabled` для сервиса `firewalld` при выключенном флаге и условная валидация firewall-переменных.
- В `base_os` установка baseline-пакетов сделана условной: пакет `firewalld` исключается из установки при `firewalld_enabled=false`.
- Документация (`Project/runbook/qa`) синхронизирована с текущим диагностическим режимом безопасности.

### Проблемы
- Требуется прогон на control host для подтверждения фактического состояния `firewalld` на всех узлах (`is-enabled/is-active`).

## Дата: 2026-03-03 (сессия 33)
### Наблюдения
- Получено требование добавить шаг обновления всех пакетов на хостах до начала установки кластера.

### Решения
- Добавлена и закрыта задача `T-076`.
- В `base_os` добавлен pre-step полного обновления установленных пакетов (`dnf update_only`) перед baseline-установкой.
- Добавлен флаг `base_os_update_all_packages` (default `true`) для управляемого включения/выключения шага.
- В inventory зафиксировано `base_os_update_all_packages: true`, документация (`Project/runbook`) синхронизирована.
...
## Дата: 2026-03-03 (сессия 37)
### Наблюдения
- Получено эксплуатационное требование выделить настройку MetalLB в отдельный сценарий запуска.
- Текущий `bootstrap.yml` включал этап MetalLB внутри общего bootstrap, что усложняло частичные прогоны.

### Решения
- Добавлен отдельный playbook `playbooks/metallb.yml` с ролью `metallb` на `control_plane[0]`.
- Этап MetalLB удален из `playbooks/bootstrap.yml`.
- В `playbooks/site.yml` добавлен отдельный импорт `metallb.yml` после `bootstrap.yml`.
- Документация (`Project.md`, `runbook.md`, `Tasktracker.md`, `changelog.md`) синхронизирована с новым порядком orchestration.
